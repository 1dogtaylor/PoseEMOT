...
path = "/Users/Lejett/Desktop/TED_Dataset/"
...
for filename in video_files:
    vibobj = cv2.VideoCapture(os.path.join(path+filename))
    ...
    while success:
        ...
        if pose_full != None and pose_vis != None and emotion != None: # if pose detected append to df
            ...
            # 'pictures' is set to None, so no need to check its length

            current_pose_df = pd.concat([current_pose_df, pd.DataFrame({'filename': filename, 'frame': i,'pose set': pose_set, 'emotion': emotion, 'pose': pose_full, 'pictures': None}, index=[0])], ignore_index=True) # append to current pose df
            current_pose_df_vis = pd.concat([current_pose_df_vis, pd.DataFrame({'filename': filename, 'frame': i,'pose set': pose_set, 'emotion': emotion, 'pose': pose_vis, 'pictures': None}, index=[0])], ignore_index=True)
            current_pose_df_3min = pd.concat([current_pose_df_3min, pd.DataFrame({'filename': filename, 'frame': i,'pose set': pose_set, 'emotion': emotion, 'pose': pose_vis, 'pictures': None}, index=[0])], ignore_index=True)
            ...
            if consecutive_frames >= 60: # if 60 consecutive frames of the same pose (2 seconds)
                if not toggle: # if this is the first frame of the pose set
                    vid_df_full = pd.concat([vid_df_full, current_pose_df], ignore_index=True)
                    vid_df_vis = pd.concat([vid_df_vis, current_pose_df_vis], ignore_index=True)
                    current_pose_df = pd.DataFrame(columns=['filename','frame','pose set','emotion','pose','pictures'])
                    current_pose_df_vis = pd.DataFrame(columns=['filename','frame','pose set','emotion','pose','pictures'])
                else: # if this is not the first frame of the pose set
                    vid_df_full = pd.concat([vid_df_full, pd.DataFrame({'filename': filename, 'frame': i,'pose set': pose_set, 'emotion': emotion, 'pose': pose_full, 'pictures': None}, index=[0])], ignore_index=True)
                    vid_df_vis = pd.concat([vid_df_vis, pd.DataFrame({'filename': filename, 'frame': i,'pose set': pose_set, 'emotion': emotion, 'pose': pose_vis, 'pictures': None}, index=[0])], ignore_index=True)
                toggle = True
            if consecutive_min_3_frames >= 60:
                if not toggle_3min:
                    vid_df_3min = pd.concat([vid_df_3min, current_pose_df_3min], ignore_index=True)
                    current_pose_df_3min = pd.DataFrame(columns=['filename','frame','pose set','emotion','pose','pictures'])
                else:
                    vid_df_3min = pd.concat([vid_df_3min, pd.DataFrame({'filename': filename, 'frame': i,'pose set': pose_set, 'emotion': emotion, 'pose': pose_vis, 'pictures': None}, index=[0])], ignore_index=True)
                toggle_3min = True
        else:
    ...
    # append to full df
    df_full = pd.concat([df_full, vid_df_full], ignore_index=True)
    df_vis = pd.concat([df_vis, vid_df_vis], ignore_index=True)
    df_3min = pd.concat([df_3min, vid_df_3min], ignore_index=True)
    videos_processed += 1
...
video_files_df.to_pickle(path+'video_files5.pkl')
