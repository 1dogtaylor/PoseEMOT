...
# Modified Data loading and preprocessing to include a test set
def load_and_preprocess_data():
    train_x = np.load(os.path.join(DATA_PATH, "train_x.npy"))
    train_y = np.load(os.path.join(DATA_PATH, "one_hot_labels.npy"))

    # First split to separate out the test set
    initial_train_x, test_x, initial_train_y, test_y = train_test_split(
        train_x, train_y, test_size=0.2, random_state=42)

    # Second split to create the training and validation sets
    train_x, val_x, train_y, val_y = train_test_split(
        initial_train_x, initial_train_y, test_size=0.25, random_state=42)  # 0.25 x 0.8 = 0.2
...
if __name__ == "__main__":
    shuf_train_x, shuf_val_x, shuf_train_y, shuf_val_y, test_x, test_y = load_and_preprocess_data()
    ...
    save_summary(shuf_model, shuf_history, shuf_tuner, "RLR LSTM")
        # After the best model is trained and saved, evaluate it on the test set
    best_model = load_model(os.path.join(SAVE_DIR_PATH, 'best_bilstm_2_model.h5'))
